...
public static class Helper
{
    ...
    public static BCEMessage ToBCEMessage(this QueuePayload<PositionData> msg)
    {
        var data = msg.Data;
        _ = long.TryParse(data.Device.UniqueId, out var imei);

        return new BCEMessage
        {
            //DeviceId = imei,
            Gps = new Gps
            {
                SatellitesFix = data.Position.Attributes.Sat,
                Speed = (int)(data.Position?.Speed ?? 0),
                Location = new Location { Lat = data.Position.Latitude, Lon = data.Position.Longitude },
                Odometer = data.Position.Attributes.Odometer,
                Altitude = (int)data.Position.Altitude
            },
            Device = new BCEDevice
            {
                Battery = (int)data.Position.Attributes.Battery,
                Cellular = new Cellular
                {
                    Rssi = data.Position.Attributes.RSSI
                }
            },
            Event = new Event
            {
            ...
            {
                IgnitionStatus = data.Position.Attributes.Ignition,
                AltitudeGps = (int)data.Position.Altitude,
                SatellitesFix = data.Position.Attributes.Sat,
                Hdop = (float)data.Position.Attributes.Hdop,
                TotalDistance = (int)data.Position.Attributes.TotalDistance,
